...
const earthMaterial = new THREE.MeshStandardMaterial( { 
  ...
  // 雲的球比地球大一點
  const cloudGeometry = new THREE.SphereGeometry(5.20,60,60)
...
const addInnerGlow = () => {
  ...
  const geo = new THREE.SphereGeometry(5.19, 50, 50);
...
const innerGlow = addInnerGlow();

const addOuterGlow = () => {
	const vertex = document.getElementById('vertexShader').innerHTML
	const fragment = document.getElementById('fragmentShaderOuterGlow').innerHTML
	const geo = new THREE.SphereGeometry(5.8, 60, 60)
  const myPosition = control.object.position.clone().normalize()
  const mat = new THREE.ShaderMaterial({
			transparent: true,
			blending: THREE.AdditiveBlending,
...
const onCameraChange = (event) => {
  ...
  innerGlow.material.uniforms.orbitcontrolPosition.value = cameraPosition
  outerGlow.material.uniforms.orbitcontrolPosition.value = cameraPosition
}
